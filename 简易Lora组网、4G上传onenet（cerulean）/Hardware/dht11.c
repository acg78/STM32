...
void DHT11_GPIO_OUT(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA,ENABLE);    //开启挂载GPIO的总线APB2
...
uint8_t DHT_Read(void)
{
	...
	DHT11_GPIO_OUT();
	GPIO_ResetBits(GPIOA,GPIO_Pin_5);//拉低引脚输出电平
	Delay_ms(18);//保证dht11能收到起始信号
	
	
	
	GPIO_SetBits(GPIOA,GPIO_Pin_5);//拉高引脚输出电平
	...
	{	//第一个循环消耗响应低电平60us
		while(GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_5)==0&&retry<100)//接收响应信号低电平剩余60us
		{
			Delay_us(1);
			retry++;		
		}
		...
		//消耗dht11拉高80us
		while(GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_5)==1&&retry<100)//接收响应信号高电平80us
		{
			Delay_us(1);
			retry++;			
		}
...
//		while(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_14)==0&&retry<100)//消耗拉低总线的50us
//		{
//			SysTick_Delay_us(1);
//			retry++;			
//		}
