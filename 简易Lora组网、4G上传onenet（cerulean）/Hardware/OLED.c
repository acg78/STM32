...
//因为是模拟IIC，经典IIC通信需要接入两个上拉电阻来实现高电平（因为设置为开漏输出，产生不了高电平）
/*引脚配置*/
#define OLED_W_SCL(x)		GPIO_WriteBit(GPIOB, GPIO_Pin_8, (BitAction)(x))//对单个引脚置1或0
...
{
	OLED_I2C_Start();
	OLED_I2C_SendByte(0x78);		//OLED从机地址固定
...
void OLED_Ground()
{      	
	static u8 pos=0;//当前位置，就在本很函数内使用，只要main不结束，他就不会被释放
	u8 speed=5;//移动的速度
